...
def dedup(path, pattern, dry_run=True, verb=False):
    ...
    for fname in fnames:
        if not is_utf(str(fname)): fnames.remove(fname)
    ...
    for fname in fnames:
        ...
        for other_form in other_forms:
            ...
            if is_norm and not is_other:
                if dry_run: 
                    if verb: 
                        print(f'File {fname_str=} is in {norm_form=} and does not exist in {other_form=}, all is fine and do nothing.')
            elif is_other:
                if is_norm :
                    if filecmp.cmp(fname_str, other_fname):
                        if dry_run: 
                            ...
                            Path(other_fname).unlink(missing_ok=True)
                    else:
